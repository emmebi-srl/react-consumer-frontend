...
import { useInvoicePayment, useSaveInvoicePayment } from '~/proxies/aries-proxy/invoice-payments';
import { usePaymentTypes } from '~/proxies/aries-proxy/payment-types';

interface InvoicePaymentModalProps extends ModalProps {
...
const InvoicePaymentModal = (props: InvoicePaymentModalProps) => {
  const snackbar = useSnackbar();
  const theme = useTheme();
  const isMobileDialog = useMediaQuery(theme.breakpoints.down('sm'));
  const paymentQuery = useInvoicePayment(props.year, props.id, props.paymentId);
  const paymentTypesQuery = usePaymentTypes();
  const employeesQuery = useEmployees();
  const saveMutation = useSaveInvoicePayment();
  ...
  return (
    <Dialog open onClose={close} maxWidth="sm" fullWidth fullScreen={isMobileDialog}>
      ...
      <InvoicePaymentForm
        ...
        heading={props.title ?? `Fattura ${props.id}/${props.year}`}
